...
spark = SparkSession.builder.appName("PySpark_SVC").getOrCreate()
...
if __name__ == "__main__":
    ...
    # now, we develop support vector classifier
    ...
    # we define the svc
    svc = LinearSVC(labelCol=target_column, featuresCol="features")
    ...
    param_grid = (ParamGridBuilder()
                  ...
                  .addGrid(svc.regParam, [0.1, 0.01, 1.0])
                  .build())
    ...
    svc_model = crossval.fit(train_data)

    # we get maxIter and regParam (best combination from our grid) of the best model
    best_max_iter = svc_model.bestModel.getMaxIter()
    best_reg_param = svc_model.bestModel.getRegParam()
    print(f"MaxIter: {best_max_iter} - RegParam: {best_reg_param}")
    # we evaluate the accuracy based on predictions on test set
    predictions = svc_model.transform(test_data) # predictions of the model on new test data
    ...
    feature_importances = list(zip(feature_columns, svc_model.bestModel.coefficients.toArray()))
